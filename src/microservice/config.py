...
STATUS_NOTE_TEMPLATES = {
    # 1. LLM распознал
    "LLM_RECOGNIZED": (
        "🤖 Сделка создана автоматически по файлу тендера.\n"
        "✔️ Что сделано: заполнены поля направления, приоритета, типа ситуации и заказчика.\n"
        "📌 Что делать: проверь правильность классификации (направление, приоритет, тип ситуации).\n"
        "➡️ Дальше: если всё верно — переведи в «Проверка Сотрудника 2» (если нужна квалификация) или сразу в «Передано в закупку» (если тендер понятен)."
    ),
    # 2. Проверка Сотрудника 2
    "CHECK_EMPLOYEE2": (
        "🔍 Сделка на квалификации — неочевидно направление или тип ситуации.\n"
        "📌 Что делать: разобраться в тендере, уточнить направление и приоритет.\n"
        "➡️ Дальше: если наш ассортимент — переведи в «Передано в закупку» или «СОЗ — звонок». Если не наш — «К архивированию» (заполни причину закрытия)."
    ),
    # 3. СОЗ — звонок
    "SOZ_CALL": (
        "📞 Сделка передана на прозвон — это СОЗ (сбор оферт запроса), дата реальных торгов неизвестна.\n"
        "📌 Что делать: позвонить заказчику, узнать дату реальных торгов и предмет закупки.\n"
        "➡️ Дальше: если торги скоро — переведи в «СОЗ — ждём дату». Если торги уже идут — «Передано в закупку»."
    ),
    # 4. СОЗ — ждём дату
    "SOZ_WAIT": (
        "⏳ Сделка в ожидании — звонок сделан, дата реальных торгов зафиксирована.\n"
        "📌 Что делать: ждём когда заказчик объявит реальные торги. Следить за площадкой.\n"
        "➡️ Дальше: как только торги объявлены — переведи в «Передано в закупку» (постави дату подачи)."
    ),
    # 5. Передано в закупку
    "PURCHASING": (
        "📦 Сделка передана в закупку / расчёт.\n"
        "📌 Что делать: рассчитать себестоимость, проверить наличие у поставщиков, подготовить КП (срок — см. поле «Срок подачи»).\n"
        "➡️ Дальше: как КП готово — переведи в «КП готовится» (если нужно внутреннее согласование) или сразу в «КП передано дилеру»."
    ),
    # 6. КП готовится
    "KP_PREPARING": (
        "📝 КП готово — проверяем цены и условия перед отправкой.\n"
        "📌 Что делать: убедиться что цены актуальны, все позиции включены, документы сформированы.\n"
        "➡️ Дальше: отправь КП дилеру и переведи в «КП передано дилеру» (заполни поле «Дилер / канал продаж»)."
    ),
    # 7. КП передано дилеру
    "KP_SENT_DEALER": (
        "📤 КП отправлено дилеру — ждём решения.\n"
        "📌 Что делать: дождаться ответа дилера — выходит на торги или нет.\n"
        "➡️ Дальше: получив ответ — заполни поле «Решение дилера» и переведи в «Решение дилера» (выходим) или «К архивированию» (не выходим)."
    ),
    # 8. Решение дилера
    "DEALER_DECISION": (
        "🤝 Решение дилера получено.\n"
        "📌 Что делать: заполни поле «Решение дилера» (Выходим на торги / Не выходим / Не ответил / Торги отменены).\n"
        "➡️ Дальше: если выходим — переведи в «Производство / Торги». Если нет — в «К архивированию» (заполни причину закрытия)."
    ),
    # 9. Производство / Торги
    "PRODUCTION": (
        "⚔️ Выходим на торги!\n"
        "📌 Что делать: контролировать ход торгов, зафиксировать результат.\n"
        "➡️ Дальше: после торгов — переведи в «К архивированию» и заполни: «Причина закрытия» (Выиграли / Проиграли / Не участвовали) и «Архивное назначение итоговое»."
    ),
    # 10. К архивированию
    "TO_ARCHIVE": (
        "🗂️ Сделка отмечена к архивированию.\n"
        "📌 Что делать: заполни обязательные поля для архивации:\n"
        ...
        "  — «Дата возврата из архива» (если нужно вернуться позже)\n"
        "⏰ Микросервис автоматически перенесёт сделку в архивную воронку ночью (если все поля заполнены)."
    ),
}
