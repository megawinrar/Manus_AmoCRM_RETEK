# SESSION STATE — RETEK amoCRM Microservice

> Последнее обновление: 10.06.2026 (PATCH18)
> Коммит: PATCH18 — Действие 3 полностью: action3_handler.py + защита от цикла
> Репозиторий: https://github.com/megawinrar/Manus_AmoCRM_RETEK

---

## Что уже сделано (полностью)

### 1. amoCRM — настройка
- Внешняя интеграция (JWT до 29.07.2026)
- 3 воронки: Активная (11 статусов) + 2 архивные
- 32 кастомных поля сделки
- Вебхук-обработчик (webhook_handler.py)

### 2. Микросервис — ядро
- `main.py` — FastAPI + APScheduler (5 cron-задач)
- `config.py` — все правила маршрутизации, статусы, задачи
- `amo_client.py` — API-клиент с rate-limit и retry
- `webhook_handler.py` — обработка вебхуков amoCRM
- `cron_hourly.py` — контроль Р1/Р2, просрочки
- `cron_daily.py` — архивация (ночной перенос)
- `cron_weekly.py` — зависшие карточки >7 дней
- `cron_monthly.py` — ревизия архива

### 3. Яндекс.Диск — сканер
- `cron_yadisk.py` — опрос /ТОРГИ каждый час
- Рекурсивный обход папок (до 3 уровней)
- ZIP-распаковка перед анализом
- Дедупликация через SQLite
- OAuth-токен работает ✅

### 4. LLM-классификатор
- `llm_classifier.py` — Яндекс GPT (OpenAI-compatible)
- Действие 1: Классификация тендера
- Действие 2: Архивное назначение (ночной cron)
- Действие 3: Распознавание из чата (заготовка)
- RAG: CONTEXT_FULL.md + amocrm_fields_and_logic.md как system prompt
- Режим: training (логирует, НЕ применяет)

### 5. Дедупликация + Валидация
- `deduplication.py` — SHA-256, fuzzy-match, обогащение
- `field_validator.py` — обязательные поля по статусам, warn/block
- 4 сценария: дубль / обогащение / обновление / fuzzy-предупреждение

### 6. Тесты — 78 штук (все зелёные)
- test_microservice.py (29)
- test_deduplication.py (35)
- test_amo_api.py (14)

### 7. Документация
- README.md — архитектура, быстрый старт
- SETUP.md — инструкция по настройке
- docs/CONTEXT_FULL.md — RAG-контекст для LLM
- docs/amocrm_fields_and_logic.md — поля и бизнес-логика
- docs/STATUS_MODEL.md — модель статусов

### 8. Эмуляция LLM — Тендер 1 (Gesac, PATCH6-8)
- Протестировано на реальных файлах (Gesac 86 поз., КБП Ростех)
- Сделка создана в amoCRM: ID 3153625 → перенесена в архив (дубль)
- Основная сделка: ID 3156121 (НПО Высокоточные комплексы)
- Классификация: CARBIDE-STANDARD, Р1 (автоэскалация с Р2, дедлайн 10.06 ≤ 48ч)
- Название: `[P1] СРОЧНО — НПО Высокоточные — 10.06`
- Тег: P1_СРОЧНО (красный)
- Post-create hook: статусная заметка + задача на 2ч ✓
- Дедупликация: SQLite + API amoCRM ✓

### 9. Эмуляция LLM — Тендер 2 (Promatool, PATCH9)
- **Сделка:** ID 3160177 — https://tokutools.amocrm.ru/leads/detail/3160177
- **Заказчик:** АО «ЦНИИ «Буревестник»» (Ростех), г. Нижний Новгород
- **Предмет:** Поставка инструмента Promatool или эквивалент (98 поз., 6245 шт)
- **НМЦ:** 49 960 625,50 руб.
- **Номер закупки:** 2109-2026-00743 / извещение 32616091745
- **Способ:** Закрытый запрос котировок (ЗЗК)
- **Дедлайн подачи:** 17.06.2026
- **Классификация:** CARBIDE-STANDARD, Р2 (Быстрые Деньги)
- **Эскалация:** НЕ требуется (дедлайн ~8 дней, > 48ч)
- **Маршрутизация:** Статус 5 «Передано в закупку / расчёт» → Сотрудник 3
- **Название:** `[P2] — ЦНИИ Буревестник — 17.06`
- **Тег:** P2_Быстрые (розовый, ID 32303)
- **Задача:** ID 1096707, дедлайн 48ч — «Рассчитать себестоимость, подготовить КП»
- **Статусная заметка:** «📦 В закупке. Считай КП к дате подачи.»
- **Дедупликация:** SQLite записано (6 файлов захешировано) ✓
- **Пометка:** Требуется ГИСП (gisp.gov.ru) — номер реестровой записи
- **Состав:** 80 фрез (5615 шт) + 5 граверов (500 шт) + 13 разверток (130 шт)

### 10. Дедупликация + обогащение при эмуляции (PATCH6)
- `scripts/emulate_llm_with_dedup.py` — полный цикл с 3 уровнями проверки
- Точный дубль → СТОП | Обогащение → PATCH + заметка + задача | Fuzzy → предупреждение
- Снимки до/после обогащения сохраняются в `data/backups/snapshots/`
- Протестировано: 3 прогона подряд → дубль → обогащение → СТОП ✓

### 11. Модуль бэкапа (PATCH6)
- `src/microservice/cron_backup.py` — полный дамп всех сделок + задач
- Сохранение: локально + Яндекс.Диск (`/ТОРГИ/_BACKUP/`)
- Ротация: хранит последние 30 бэкапов
- Снимки отдельных сделок (до/после обогащения)
- Первый бэкап: 6 сделок, 4 задачи (38 KB)

### 12. Post-create hook + автоэскалация (PATCH7-8)
- `post_create_hook()` в `emulate_llm_with_dedup.py` — срабатывает после создания/обогащения
- Автоэскалация: deadline ≤ 48ч → Р1 (перебивает Р2/Р3)
- Переименование: `[P1] СРОЧНО — Заказчик — DD.MM` (без эмодзи — amoCRM стрипит 4-байтный UTF-8)
- Цветные теги: P1_СРОЧНО (красный), P2 (жёлтый), P3 (зелёный), P4 (серый)
- Статусная заметка: первые 44 символа видны на канбане
- Задача: Р1 → 2ч | Р2 → 2 дня | Р3 → по правилу статуса
- `cron_hourly.py` — эскалация по дедлайну (проверка каждый час)
- Важно: amoCRM не поддерживает DELETE заметок, но PATCH работает
- Важно: эмодзи (4-байт UTF-8) в поле name сбрасывают название на "Сделка #ID"

### Цветные теги (созданы в amoCRM)
| Тег | ID | Цвет |
|---|---|---|
| P1_СРОЧНО | 31613 | #FF8F92 (красный) |
| P2_БыстрыеДеньги | 31615 | #FFC8C8 (жёлтый) |
| P3_СредниеДеньги | 31617 | #DBFFB0 (зелёный) |
| P4_Наблюдаем | 31619 | #D6D6D6 (серый) |

### Обработанные сделки (итого)
| # | Сделка ID | Заказчик | НМЦ | Приоритет | Статус |
|---|---|---|---|---|---|
| 1 | 3156121 | НПО Высокоточные комплексы | 6 719 075,91 | Р1 | 5. Передано в закупку |
| 2 | 3153625 | (дубль → архив) | — | Р4 | Архив — дубли |
| 3 | 3160177 | ЦНИИ Буревестник | 49 960 625,50 | Р2 | 5. Передано в закупку |

### 13. Быстрая экстракция + парсинг (PATCH10-12)
- `scripts/extract_and_classify.py` — полный цикл экстракции + парсинга тендерных файлов
- **Время:** ~2.5 сек на 6 файлов (9 MB, 640K символов)
- **Экстракция:** pdftotext / python-docx / openpyxl
- **OCR-fallback:** pytesseract + pdf2image (только когда нужно)
- **Детекция сканов:** < 100 симв/стр → авто-OCR
- **Валидация:** 13 полей с confidence scoring (0.0–1.0)
- **OCR-верификация:** точечный OCR региона вокруг ключевого слова при confidence < 0.7
- **Статусы:** `ok` | `warnings` | `blocked`
- **Режимы вывода:** терминал / `--json` / `--command` (генерация команды для emulate_llm_with_dedup.py)

### 14. RAG-система (PATCH13)
- `scripts/rag_indexer.py` — чанкование (250 токенов, overlap 75) + concurrent embedding + ChromaDB
- `scripts/rag_search.py` — семантический поиск с keyword boost, query expansion, top_k=10
- `scripts/pipeline_tender.py` — интеграционный пайплайн: extract → classify → index
- Яндекс Embeddings API работает (256 dim, text-search-doc/latest)
- Тендер Promatool: 1058 чанков, стоимость индексации ~1.81₽
- Стоимость поиска: ~0.001₽/запрос
- ChromaDB база: `data/chroma_db/`

### 15. Деплой-артефакты (PATCH14-15)
- `deploy/Dockerfile` — production образ (python:3.11-slim + tesseract + poppler)
- `deploy/docker-compose.yml` — 3 сервиса: app + nginx + certbot
- `deploy/nginx/nginx.conf` + `deploy/nginx/conf.d/app.conf` — reverse proxy
- `deploy/setup_vm.sh` — первоначальная настройка VM
- `deploy/create_env.sh` — шаблон .env с PLACEHOLDER (без секретов в git)

### 16. Production деплой на Yandex Cloud VM (PATCH16) ✅
- **VM:** 89.169.142.160 (Ubuntu 22.04, Docker 29.5.3)
- **Репозиторий на VM:** /opt/Manus_AmoCRM_RETEK
- **Статус контейнеров:**
  - `retek-amocrm` — Up (healthy) ✅
  - `retek-nginx` — Up (порты 80, 443) ✅
  - `retek-certbot` — Up ✅
- **Health check:** `http://89.169.142.160/health` → `{"status":"ok"}` ✅
- **Webhook:** `http://89.169.142.160/webhook` (POST) → готов к приёму ✅
- **Режим:** DRY_RUN=0, LLM_MODE=training
- **Cron-задачи запущены:** 5 задач (APScheduler)
- **Webhook URL для amoCRM:** `http://89.169.142.160/webhook`

### 17. Интеграция webhook + extract_and_classify + Действие 3 (PATCH17) ✅

**Webhook amoCRM зарегистрирован через API:**
- ID: 47825646, URL: `http://89.169.142.160/webhook`
- События: `add_lead`, `update_lead`, `note_lead` ✅

**extract_and_classify.py интегрирован в cron_yadisk.py:**
- Заменяет медленный LLM-классификатор для новых тендеров
- Пайплайн: скачать файлы → extract_file() → parse_tender_fields() → validate_and_fallback()
- Выход: customer, nmc, direction, deadline, priority, validation_status, confidence
- OCR-fallback автоматически для сканированных PDF ✅

**Действие 3 — распознавание из чата amoCRM (PATCH18 — полная реализация):**
- `action3_handler.py` — полный пайплайн (~380 строк)
- **Триггеры:** ссылка на Я.Диск (yadi.sk, disk.yandex.ru, disk:/ПУТЬ) + слова ("распознай", "тендер", "parse", "extract")
- **Пайплайн:** парсинг ссылки → скачивание с Я.Диска (public API) → распаковка ZIP → extract_and_classify → заполнение полей карточки
- **Заполняемые поля:** customer, nmc, direction, priority, situation_type, procedure_number, deadline
- **Маршрутизация:** resolve_routing(priority, situation) → status_id + responsible_user_id
- **Название:** `[P{n}] — Заказчик — DD.MM`
- **Защита от цикла:** фильтр bot_own_note (префиксы 🤖/✅/❌/⚠️ + маркеры)
- **Тест:** скачивание 101 файла + распознавание прошло ✅
- **Блокер:** Токен amoCRM истёк (402), обновление карточки не сработало. Ждём ответ поддержки amoCRM

---

## Что НЕ сделано (следующие шаги)

### Задачи по приоритету

| # | Задача | Приоритет | Зависимость |
|---|---|---|---|
| 1 | Обновить токен amoCRM (ждём поддержку) | 🔴 | Блокер |
| 2 | Проверить Действие 3 end-to-end (после токена) | 🔴 | Токен |
| 3 | **CRM-Audit-Service** — реализовать MVP (см. ниже) | 🔴 | Токен |
| 4 | **Hermes TG Bot** — наблюдатель Телеграм (см. ниже) | 🟡 | После CRM-Audit |
| 5 | **Управление автоматизацией** — аналитика всех досок (см. ниже) | 🟡 | После CRM-Audit |
| 6 | Перевести LLM в production mode (LLM_MODE=production) | 🟡 | После обкатки |
| 7 | SQLite FTS5 для поиска по архиву тендеров (когда будет 50+ тендеров) | 🟢 | — |
| 8 | RAG чат в карточке amoCRM | 🟢 | Будущее |

---

### Hermes TG Bot (Read Only)

**Цель:** Telegram-бот-наблюдатель, который читает рабочие чаты, записывает в базу, делает сводку.

**Функции MVP:**
- Мониторинг рабочих чатов ТГ (личные дела, рабочие дела, файлы на просчёт)
- Запись всего в базу (кто, что, когда, какие файлы)
- Распознавание голосовых сообщений (speech-to-text)
- Сбор файлов на просчёт у поставщиков (агрегация)
- Ежедневная сухая выжимка (что было за день)
- **Режим Read Only** — только читает и записывает, не отвечает и не управляет

**Открытые вопросы (обсудить перед реализацией):**
1. Hermes подключается как **userbot** (от имени аккаунта, видит все чаты) или как **бот** (добавляется в конкретные группы)?
2. Какие именно чаты/группы мониторить? Все рабочие или конкретный список?
3. Куда отправлять ежедневную сводку — в отдельный канал/чат, в amoCRM, или в файл?
4. Какая БД для хранения (SQLite / PostgreSQL / другое)?
5. Нужно ли связывать сообщения ТГ с карточками amoCRM?

---

### Управление автоматизацией (amoCRM — все доски)

**Цель:** Аналитика и слежение за цепочкой заявок во ВСЕХ досках amoCRM (не только тендерной).

**Функции:**
- Слежение за цепочкой заявок согласно логике каждой доски
- Бэкап всей информации из всех воронок
- Слепки состояния: где что лежало, сколько времени
- Аналитика: сколько времени не исполнялось, где зависания
- Сначала Read Only (аналитика), потом — управление (перемещение, задачи, алерты)

**Связь с CRM-Audit-Service:** Это расширение CRM-Audit-Service на все воронки + добавление фазы управления.

---

### CRM-Audit-Service (репо: [CRM-Audit-Service](https://github.com/megawinrar/CRM-Audit-Service))

**Цель:** Read-only наблюдатель за amoCRM — снимки, диффы, проверка правил, отчёты.

**Статус:** Только каркас (`__init__.py`) + ТЗ (CODEX_TZ_ROADMAP.md). Код не написан.

**Шаги реализации (10 шагов по roadmap):**

| Шаг | Описание |
|---|---|
| 1 | Каркас проекта + config/rules.json + field_mapping.json |
| 2 | Normalizer — приведение сделок к единому формату |
| 3 | Snapshot storage — сохранение снимков в JSON |
| 4 | Diff engine — сравнение двух снимков |
| 5 | Rules engine — проверка нарушений логики |
| 6 | Report generator — ежедневные/недельные отчёты |
| 7 | Тесты (normalizer, diff, rules) |
| 8 | amoCRM read-only client (GET только) |
| 9 | CLI main.py (snapshot / diff / rules / report / run-once) |
| 10 | GitHub Actions workflow (ежечасный запуск) |

**Ключевые правила:**
- MVP ничего не меняет в amoCRM (read-only)
- Не хранит токены в репозитории
- Проверяет канонические правила маршрутизации
- Запуск через GitHub Actions (ежечасно)
- Артефакты: snapshots/, diffs/, reports/

**Сервис отвечает на вопросы:**
- Какие сделки зависли?
- Где нарушена маршрутизация?
- Какие сотрудники перегружены?
- Какие правила CRM игнорируются?
- Где нет следующего действия?

---

## Ключевые решения (принятые)

1. **Статусы:** 9=Торги → 10=Производство → 11=К архивированию
2. **LLM всегда работает** (не rule-based), сначала training → потом production
3. **Яндекс GPT** (не OpenAI) — через OpenAI-compatible endpoint
4. **Дедупликация:** хеш + fuzzy, обогащение при новых файлах
5. **Валидация:** warn (предупреждение) + block (для архива)
6. **ZIP-архивы:** распаковываем, анализируем содержимое
7. **Объём:** 10-20 тендеров/день
8. **Деплой:** Docker + nginx, без SSL (нет домена), webhook по IP:80

---

## Учётные данные (в .env)

| Сервис | Статус |
|---|---|
| amoCRM JWT | ❌ Истёк (402). Ждём поддержку amoCRM |
| Яндекс.Диск OAuth | ✅ Работает |
| Яндекс GPT API | ✅ Работает (ключ в .env) |
| Yandex Embeddings | ✅ Работает (256 dim) |

---

## Структура папок Яндекс.Диска

```
/ТОРГИ/
└── {DD.MM.YYYY}/
    └── {Название тендера}/
        ├── файлы (.xlsx, .docx, .pdf, .doc)
        ├── архивы (.zip) → распаковываются
        └── подпапка/ → рекурсивный обход
```

Реальный пример (09.06.2026): 17 тендеров в папке.

---

## Инфраструктура

| Компонент | Значение |
|---|---|
| VM IP | 89.169.142.160 |
| VM OS | Ubuntu 22.04 |
| Docker | 29.5.3 |
| Репозиторий на VM | /opt/Manus_AmoCRM_RETEK |
| Health URL | http://89.169.142.160/health |
| Webhook URL | http://89.169.142.160/webhook |
| amoCRM домен | tokutools.amocrm.ru |
| GitHub | https://github.com/megawinrar/Manus_AmoCRM_RETEK |
